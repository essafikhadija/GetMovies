...
const API_KEY="api_key=bb59d15ccbb1b8932591a21f241c39e2"
   ...
   applyVideoToCurrentMovie(){
    ...
    onClickListItem(movie){
       this.setState({currentMovie :movie},function (){
           this.applyVideoToCurrentMovie();
           this.setRecommandedMovies();
       });
    }
    recieveSearchWord(searchedMovie){
       if(searchedMovie){
           axios.get(`${API_END_POINT}${SEARCH_URL}&${API_KEY}&query=${searchedMovie}`).then(function (response) {
    ...
    render() {
       const renderVideoList=()=>{
           if(this.state.moviesList.length>=5){
               return(
                   <VideoList movieList={this.state.moviesList} callback={this.onClickListItem.bind(this)}/>
               )
           }
         ...
         return(
             <div>
                 <div className="search_bar"><SearchBar callback={this.recieveSearchWord.bind(this)}/></div>
                 <div className="row">
